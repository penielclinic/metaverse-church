    ...
    async function init() {
      ...
      if (cell?.mission_id === 4) {
        ...
        const queries: [string, string, Record<string, unknown>?][] = [
          ...
          ['elder_reco', 'elder_recommendations'],
          ['elder_care', 'elder_care_requests'],
          ['elder_announce', 'elder_announcements'],
          ['elder_help', 'elder_help_requests'],
        ]
        await Promise.all(queries.map(async ([key, table, filters]) => {
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center"
        ...
        <div className="w-full px-5" style={{ maxWidth: 380 }}>
          ...
          <div className={`grid grid-cols-3 gap-3 ${isElderRoom ? 'max-h-[55vh] overflow-y-auto pr-1' : ''}`}>
            {BOARDS.map((board) => {
              const badge = board.key === 'chat' ? newMsgCount : board.key === 'board' ? notes.length : board.key === 'requests' ? pendingCount : (elderCounts[board.key] ?? 0)
